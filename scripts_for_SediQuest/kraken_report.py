...
parser.add_argument('--extract-out-base',  help='Base name for output file.')
parser.add_argument('--min_length', default=0, type=int, help='Minimum length filter')
parser.add_argument('--extract-outdir', default = '.', help = 'Output directory for bam/fastas. Default is current directory.')
...
if not args.extract_outdir.endswith('/'):
    args.extract_outdir += '/'
    pass
...
def get_taxonomy_str(taxid):
    ...
    if not taxid_string:
        ...
        print('taxid_string', taxid_string)
    return taxid_string
...
def extract_bam_from_id(fileout, id_list, seqfile):
    print('extracting to file', fileout + '.bam', file=sys.stderr)
    num_seq_to_extract = len(id_list)
...
def extract_seq_from_id(fileout, id_list, seqfile, data_type='bam'):
    if seqfile.endswith("fasta") or seqfile.endswith("fa") or seqfile.endswith("fas") or seqfile.endswith("fa.gz") or seqfile.endswith("fasta.gz") or seqfile.endswith("fas.gz"):
        data_type = 'fasta'
...
def dfs_report (node, depth, related=[]):
    ...
    if args.extractFile:
        ...
        if (node in args.clades or rank_code(rank) == args.rank) and \
            ...
            # ofile_name = args.extract_outdir + \
            #              os.path.basename(args.infile)[:-suffix_length] + \
            #              "k_"+name_map[node].replace(' ','_') + \
            #              args.suffix

            # the names contains whitespaces
            print('extract_seq_from_id', args.extract_out_base, \
                                extract_ids, args.extractFile)
            extract_seq_from_id(args.extract_out_base, \
                                extract_ids, args.extractFile)
            extract_ids = set()
