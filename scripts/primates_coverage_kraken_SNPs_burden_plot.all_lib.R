...
#data <- fread("/mnt/expressions/Aurore/sediment_pipeline_v0/config/samples.csv", fill=TRUE)
...
file_list_per_indexlibid <- list()

for (indexlibid_value in data$indexlibid) {
  pattern <- paste0("^", indexlibid_value, "\\.kraken_spc$")
  file_list_mapped_bam <- list.files(mapped_bam_path, pattern = pattern, full.names = TRUE, recursive = TRUE)
  file_list_mapped_bam_with_id <- data.frame(
    indexlibid = rep(indexlibid_value, length(file_list_mapped_bam)),
    file_path = file_list_mapped_bam
  )
  file_list_per_indexlibid[[indexlibid_value]] <- file_list_mapped_bam_with_id
}

combined_file_list <- do.call(rbind, file_list_per_indexlibid)


combined_file_content <- lapply(1:nrow(combined_file_list), function(i) {
  file_data <- fread(combined_file_list$file_path[i])
  file_data$indexlibid <- combined_file_list$indexlibid[i]
  return(file_data)
})

kraken_mapped_bam <- rbindlist(combined_file_content)%>%
  ...
  mutate(step = rep(c("mapped", "deam", "unique&qual&length", "target", "raw"), length.out = n())) %>%
  pivot_longer(cols = -c(step, total, indexlibid), names_to = "Species", values_to = "Value") %>%
  mutate(Percentage = (Value / total) * 100) %>%
  select(Percentage, Species, step, indexlibid)
...
kraken_mapped_bam$step <- factor(kraken_mapped_bam$step, levels = c("raw", "mapped", "target", "unique&qual&length", "deam"))
...
plot_kraken_mapped_bam <- ggplot(kraken_mapped_bam, aes(x = step, y = Percentage, fill = Species)) +
  ...
  facet_wrap(~ indexlibid, scales = "free_y") 
...
#kraken infokraken_byread#
#ath_to_kraken <- paste0("/mnt/expressions/Aurore/sediment_pipeline_v0/output_v0/mappedbams/", data$indexlibid, "/", data$probeset, "/target/", score_b , "_filter/rmdupL35MQ25/", data$indexlibid,"_n_",n_score, ".byread")
#raken <-  fread(path_to_kraken, header=F) %>%
 #select(V1,V2) %>%
  #ename(read=V1)

#take positions and reads names assuming . value are 0 for b score
#to get the number of read for each burden scores
#ath_to_primate <- paste0("/mnt/expressions/Aurore/sediment_pipeline_v0/output_v0/mappedbams/", data$indexlibid, "/", data$probeset, "/target/", score_b , "_filter/rmdupL35MQ25/",data$indexlibid,"_n_",n_score, ".read_summary.txt.gz")
...
#save plot
#path_to_save <- paste0("/mnt/expressions/Aurore/sediment_pipeline_v0/output_v0_summary/" , data$indexlibid, "/",data$probeset, "/", score_b , "_filter/", data$indexlibid,"_n_",n_score,"_burden_SNPs_plot.pdf")
